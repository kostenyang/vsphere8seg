...
```powershell
# 1. Connect (prompts for password; connects VI + CIS + SSO)
./Tools/connect.ps1 -vCenter vcsa.example.local -User administrator@vsphere.local

# 2. Audit everything into an (empty) output directory
mkdir .\report
...
Each remediate script has a **safety block that `Exit`s before making any change**.
Read it, then comment/remove the indicated `Exit` line to opt in. Always test
outside production and take backups/snapshots first.

```powershell
# ESXi — site values passed as parameters; risky controls are opt-in switches
...
### Guest VMware Tools (`guest-8.tools-*`)
...
To push it from the vCenter side to many guests at once:

```powershell
Get-VM | Where-Object { $_.Guest.OSFullName -match 'Windows' } | ForEach-Object {
    Invoke-VMScript -VM $_ -ScriptType Powershell `
        -ScriptText (Get-Content .\Tools\guest-tools-8-vsmc-v1.ps1 -Raw) + "`nguest-tools-8-vsmc-v1 -Remediate" `
...
Provided "AS IS", without warranty, and **not supported by anyone** (carried over from
the Broadcom sample license, see `Tools/LICENSE`). Engage your own legal,
